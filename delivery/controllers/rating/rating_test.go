...
func TestRatingCreate(t *testing.T) {
	...
	t.Run("test post rating bad request validate ", func(t *testing.T) {
		e := echo.New()
		e.Validator = &rating.RatingValidator{Validator: validator.New()}
	...
	t.Run("test post rating not found err", func(t *testing.T) {
		e := echo.New()
		e.Validator = &rating.RatingValidator{Validator: validator.New()}
	...
	t.Run("test post rating err Repo.create", func(t *testing.T) {
		e := echo.New()
		e.Validator = &rating.RatingValidator{Validator: validator.New()}
		...
		ratingController := rating.NewRatingController(mockFalseRating2{})
		if err := middleware.JWT([]byte(constants.JWT_SECRET_KEY))(ratingController.Create)(context); err != nil {
			log.Fatal(err)
			return
		}
...
func (m mockRating) IsCanGiveRating(userId, transactionId int) (bool, error) {
	return true, nil
}
...
func (m mockFalseRating) IsCanGiveRating(userId, transactionId int) (bool, error) {
...
//MOCK FALSE RATING2
//==========================
type mockFalseRating2 struct{}

func (m mockFalseRating2) Create(rating models.Rating) (models.Rating, error) {
	return models.Rating{
		Rating: 5,
	}, errors.New("FAILED")
...
func (m mockFalseRating2) Update(models.Rating) (models.Rating, error) {
	return models.Rating{
		Rating: 5,
	}, errors.New("FAILED")
}

func (m mockFalseRating2) IsCanGiveRating(userId, transactionId int) (bool, error) {
	return true, nil
}

func (m mockFalseRating2) GetByTrxID(trxID int) (models.Rating, error) {
	return models.Rating{
		Rating: 5,
	}, nil
}
